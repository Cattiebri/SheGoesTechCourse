...
public class Practice {
	...
	public static int getAvarege(int[] someArray) {
		...
		// array 'someArray'
		return getSum(someArray) / someArray.length;
	}

	public static int getMaxValue(int[] someArray) {
		int myMaxValue = someArray[0];
		for (int i = 0; i < someArray.length; i++) {
			if (myMaxValue < someArray[i]) {
				myMaxValue = someArray[i];
			}
		}
		return myMaxValue;
	}
	...
	public static double getAvaregeDouble(int[] someArray) {
		...
		// double doubleSum = new Double(getSum(someArray));
		double doubleSum = (double) getSum(someArray);
		return doubleSum / someArray.length;
	}
	...
	public static void myMethod() {
		System.out
				.println("Enter a number of how big array you want to create: ");
		int size = scanner.nextInt();
		int[] newArray = getArrayDefinedByUser(size);
		printArray(newArray);
